...
def summarize_text(text, num_sentences=3):
    ...
    # Sort the words based on their frequncy in descending order
...
def main():
    ...
    if choice == "Translation":
        ...
        target_lang = st.selectbox("Target Language", ["German", "Spanish", "French", "Italian", "Korean"])
    ...
    if choice == "Sentiment Analysis":
        ...
        st.write("")
        st.write("")
        raw_text = st.text_area("Text to analyse", placeholder="Enter a text here...", height=200)
        if st.button("Evaluate"):
            if len(raw_text) == 0:
                st.warning("Enter a text...")
            else:
                blob = TextBlob(raw_text)
                st.info("Sentimen Analysis")
